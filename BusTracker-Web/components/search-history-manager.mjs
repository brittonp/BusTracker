    ...
    history = history
      ...
      .filter((item, index, self) => {
        return (
          ...
          self.findIndex(
            (r) =>
              r.operatorRef === item.operatorRef && r.lineRef === item.lineRef
          )
        );
      })
      .map((item, index, self) => {
        const operator = operatorRoutes.list.find(
          (o) => o.operatorRef == item.operatorRef
        );

        if (item.lineRef) {
          const route = operatorRoutes.list.find(
            (r) =>
              r.operatorRef == item.operatorRef && r.lineRef == item.lineRef
          );
