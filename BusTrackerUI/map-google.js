...
export let mapObj = {
    ...
    create: async function (mapID, center) {
        ...
        this.map.addListener('dragend', () => {
            if (this.currentViewMode == appConstant.viewMode.search) {
                this.mapElement.dispatchEvent(new Event('map-move'));
            }
        });
    ...
    addVehicles: async function (vehicles) {
        ...
        $.each(vehicles, function (index, vehicle) {
            ...
            tag.innerHTML = `<div class="bt-marker-vehicle bus-direction-${vehicle.extendedAttributes.directionCode} ${favourite} ${aged}" style="--dir: ${Number(vehicle.MonitoredVehicleJourney.Bearing)}"><div class="route">
                    ${appUtils.stringTrim(vehicle.MonitoredVehicleJourney.PublishedLineName, 3)}</div>
                </div>`;
            ...
            marker.addListener('click', (e) => {
                ...
                $(info.element)
                    .on('click', '.route-link', (e) => {
                        $(thisObj.mapElement).trigger('search-route', JSON.parse(e.currentTarget.attributes.data.value));
                    })
                    .on('click', '.favourite-link', (e) => {
                        $(thisObj.mapElement).trigger('add-favourite', e.currentTarget.attributes.data.value);
                    })
                    .on('click', '.track-link', (e) => {
                        $(thisObj.mapElement).trigger('track-vehicle', e.currentTarget.attributes.data.value);
                    })
                    .on('click', (e) => {
                        info.setMap(null);
                    })
    ...
    addTrackedVehicle: async function (vehicle) {
        ...
        if (this.markers.length > 0) {
            if (JSON.stringify(this.markers[this.markers.length - 1].vehicle.MonitoredVehicleJourney.VehicleLocation) === JSON.stringify(vehicle.MonitoredVehicleJourney.VehicleLocation)) {
                return;
            }
        }
        ...
        timeTag.innerHTML = `<div class="bt-time">${appConstant.timeENGFormatter.format(new Date(vehicle.RecordedAtTime))}</div>`;
        ...
        if (this.markers.length > 1) {
            ...
            let prevPoint =
            {
                ...
                lat: Number(this.markers[this.markers.length - 2].vehicle.MonitoredVehicleJourney.VehicleLocation.Latitude),
                lng: Number(this.markers[this.markers.length - 2].vehicle.MonitoredVehicleJourney.VehicleLocation.Longitude)
            };

            let currentPoint = {
                timestamp: this.markers[this.markers.length - 1].vehicle.RecordedAtTime,
                lat: Number(this.markers[this.markers.length - 1].vehicle.MonitoredVehicleJourney.VehicleLocation.Latitude),
                lng: Number(this.markers[this.markers.length - 1].vehicle.MonitoredVehicleJourney.VehicleLocation.Longitude)
            };
            ...
            const path = new google.maps.Polyline({
                ...
                geodesic: true,
                strokeColor: window.getComputedStyle(document.body).getPropertyValue('--tracker-line-colour'),
                strokeWeight: window.getComputedStyle(document.body).getPropertyValue('--tracker-line-weight'),
                strokeOpacity: window.getComputedStyle(document.body).getPropertyValue('--tracker-line-opacity'),
                icons: [
                    {
                        icon: lineSymbol,
                        offset: '65%',
            ...
            speedTag.innerHTML = `<div class="bt-speed">${Math.floor(velocity.speedMph).toString()} mph</div>`;
        ...
        this.flyTo(position, (this.markers.length > 1) ? this.getZoom() : appConstant.defaultZoom);
